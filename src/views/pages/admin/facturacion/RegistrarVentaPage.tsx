...
const RegistrarVentaPage = () => {
  ...
  useEffect(() => { clienteService.obtenerTodos().then(setClientes).catch(() => {}); }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <CardBox className="shadow-none border-2 border-success/20 bg-gradient-to-b from-success/5 to-transparent">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-4">
          <div className="flex flex-col gap-1.5"><Label>Descuento global (%)</Label><Input type="number" min={0} max={100} value={form.descuentoGlobalPorcentaje} onChange={e => setForm({...form, descuentoGlobalPorcentaje: Number(e.target.value)})} /></div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border">
          ...
          <Button onClick={handle} disabled={g} className="bg-success hover:bg-success/90">{g ? 'Registrando...' : 'Registrar venta'}</Button>
        </div>
      </CardBox>
    </div>
  );
